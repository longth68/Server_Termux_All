...
if (isset($jsonBody->callback_sign) && $jsonBody->callback_sign == md5($Partner_Key . $jsonBody->code . $jsonBody->serial)) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $Status_Update = $conn->prepare("UPDATE napthe SET status = :status WHERE code = :code AND serial = :serial AND status = 99");
        if ($Status_Update->execute([':status' => $jsonBody->status, ':code' => $code, ':serial' => $serial])) {
            if ($jsonBody->status == 1 && $Status_Update->rowCount() === 1) {
                $Account_Update = $conn->prepare("UPDATE accounts SET vnd = vnd + :price, tongnap = tongnap + :amount WHERE user = :user_nap");
                if (!$Account_Update->execute([':price' => $price, ':amount' => $row['amount'], ':user_nap' => $user_nap])) {
