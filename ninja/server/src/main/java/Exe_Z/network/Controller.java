...
private static final ConcurrentHashMap<Byte, Integer> updateDataCounts = new ConcurrentHashMap<>();
...
public void messageNotMap(Message mss) {
    if (mss != null) {
        try {
            ...
            // Kiểm tra và chống spam các command update
            if (command == CMD.UPDATE_DATA || command == CMD.UPDATE_MAP 
                || command == CMD.UPDATE_SKILL || command == CMD.UPDATE_ITEM) {
                
                int count = updateDataCounts.getOrDefault(command, 0);
                System.out.println("Player " + user.id + ", command " + command + ", count: " + count);

                if (count >= 3) {
                    System.out.println("Disconnecting player " + user.id + " due to excessive command " 
                                      + command + " (count: " + count + ")");
                    client.disconnect();
                    updateDataCounts.clear();
                    return;
                }

                updateDataCounts.put(command, count + 1);
            }
                switch (command) {
