...
public class Mount extends Item {
    ...
    public void fixMount() {
        if ((this.id == ItemName.HUYET_SAC_HUNG_LANG || this.id == ItemName.XICH_NHAN_NGAN_LANG || this.id == ItemName.XE_MAY || this.id == ItemName.HARLEY_DAVIDSON ||this.id == ItemName.LAN_SU_VU 
                || this.id == ItemName.HO_LON || this.id == ItemName.HOA_KY_LAN || this.id == ItemName.BACH_HO || this.id == ItemName.PHUONG_HOANG_BANG) && this.isForever()
                && this.sys > 1) {
            fixMountOption();
        }
    }
