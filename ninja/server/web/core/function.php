...
function user($find, $__value = NULL)
{
    if ($__value == NULL) {
        if (is_login()) {
            $result = __query("SELECT * FROM username WHERE player = '" . $_SESSION['user'] . "'");
            if ($result) {
        ...
        $result = __query("SELECT * FROM player WHERE id = $__value");
        if ($result) {
            $user = $result->fetch_array();
            return isset($user[$find]) ? $user[$find] : null;
        }
...
function getQrMomoPayment($username, $amount, $acctNum){
    return "https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=2|99|$acctNum|||0|0|$amount|nt $username|transfer_p2p";
}
