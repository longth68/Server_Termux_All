...
# ------------------------------------------------------------
# 5. Ngọc Rồng - hashirama
# ------------------------------------------------------------
NRO_OK=0
NRO_TBLS="$(table_count hashirama)"
if [ -n "$NRO_TBLS" ] && [ "$NRO_TBLS" -gt 0 ] 2>/dev/null; then
    info "DB hashirama đã có dữ liệu ($NRO_TBLS table)."
    NRO_OK=1
else
    NRO_SQL=""
    for f in \
        "$DIR/nro/database/hashirama.sql" \
        "$DIR/nro/hashirama.sql"
    do
        if [ -f "$f" ]; then
            NRO_SQL="$f"
            break
    ...
    if [ -n "$NRO_SQL" ]; then
        import_db "hashirama" "Ngọc Rồng" "$NRO_SQL" && NRO_OK=1
    else
...
if [ "$NINJA_OK" -eq 1 ] && [ "$HTTH_OK" -eq 1 ] && [ "$NRO_OK" -eq 1 ]; then
    ...
    info "DB schoolzz (Ninja) + DB htth (Hải Tặc Hot) + DB hashirama (Ngọc Rồng) đều OK."
    exit 0
fi
