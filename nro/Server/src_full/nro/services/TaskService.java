...
public class TaskService {
    ...
    public void sendNextTaskMain(Player player) {
        try { rewardDoneTask(player); } catch (Exception ignored) {}
        ...
        try { sendTaskMain(player); } catch (Exception ignored) {}
        try {
...
//            DatabaseUpdater.updateLastTimeUpdateTask(player, System.currentTimeMillis());
            ...
            try { PlayerService.gI().sendInfoHpMpMoney(player); } catch (Exception ignored) {}
            } catch (Exception ignored) {}
            return true;
        }
        return false;
    ...
    public void addDoneSubTask(Player player, int numDone) {
        ...
        if (player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count
            ...
            if (player.playerTask.taskMain.index >= player.playerTask.taskMain.subTasks.size()) {
                try { this.sendNextTaskMain(player); } catch (Exception ignored) {}
            } else {
                try { this.sendNextSubTask(player); } catch (Exception ignored) {}
            }
        } else {
            try { this.sendUpdateCountSubTask(player); } catch (Exception ignored) {}
        }
    }
