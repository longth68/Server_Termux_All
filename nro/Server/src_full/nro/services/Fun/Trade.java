...
public class Trade {
    ...
    /** Giới hạn tổng lưu lượng vàng giao dịch mỗi người mỗi ngày (chống rửa vàng). */
    public static final long MAX_GOLD_TRADE_DAY = 2_000_000_000L;
    ...
    private void startTrade() {
        ...
        // ===== Giới hạn vàng giao dịch mỗi ngày (chống rửa vàng - port hashirama đã sửa lỗi) =====
        player1.lazyResetTradeDay();
        player2.lazyResetTradeDay();
        if (tradeStatus == SUCCESS) {
            if (player1.maxGoldTradeDay + goldTrade1 + goldTrade2 > MAX_GOLD_TRADE_DAY) {
                tradeStatus = FAIL_MAX_GOLD_PLAYER1;
                Service.gI().sendThongBao(player1, "|7|Bạn đã vượt giới hạn giao dịch "
                        + Util.format(MAX_GOLD_TRADE_DAY) + " vàng trong ngày!");
                Service.gI().sendThongBao(player2, "|7|Đối phương đã vượt giới hạn giao dịch trong ngày!");
            } else if (player2.maxGoldTradeDay + goldTrade1 + goldTrade2 > MAX_GOLD_TRADE_DAY) {
                tradeStatus = FAIL_MAX_GOLD_PLAYER2;
                Service.gI().sendThongBao(player2, "|7|Bạn đã vượt giới hạn giao dịch "
                        + Util.format(MAX_GOLD_TRADE_DAY) + " vàng trong ngày!");
                Service.gI().sendThongBao(player1, "|7|Đối phương đã vượt giới hạn giao dịch trong ngày!");
            }
        }
        ...
        if (tradeStatus != SUCCESS) {
            ...
            if (tradeStatus != SUCCESS) {
                ...
                if (tradeStatus == SUCCESS) {
                    ...
                    // Cộng dồn lưu lượng vàng giao dịch trong ngày
                    player1.maxGoldTradeDay += goldTrade1 + goldTrade2;
                    player2.maxGoldTradeDay += goldTrade1 + goldTrade2;
